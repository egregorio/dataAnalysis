...
arm_length = x_918_mean[switch_S3]
alpha = x_918_mean / arm_length
angle_918 = np.arcsin(alpha) * (180 / np.pi)
std_918 = np.arcsin(x_918_std / arm_length) * (180 / np.pi)
angle_918[switch_S3:] = 180 - angle_918[switch_S3:]

time_404 = data_04[:,0] * spring_shape

path_918 = '/Users/elizabeth/egregorio@gwmail.gwu.edu - Google Drive/My Drive/Box Sync/dataAnalysis/current video'
...
for i in range(0,len(time_918)):
	...
	ax1.plot(time_918,y_918_mean,color='seagreen',label='W22S3')
	ax1.plot(time_918[i],y_918_mean[i],'go')
	ax1.set_xlim(0,2.5)
	#ax2.set_title('Angle of Roll After Impact')
	ax2.plot(time_918,angle_918,color='seagreen',label='W22S3')
