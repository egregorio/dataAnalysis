...
#	for i in range(0,len(first_index_list)):
#		for j in range(0,x_max_length):
#			padded_x[i][:] = padded_x[i][:] - padded_x[i][0]	
#			padded_y[i][:] = padded_y[i][:] - padded_y[i][0]	
#			padded_x[i][0] = 0
#			padded_y[i][0] = 0
#			at90 = np.argmin(padded_x[i][:])
#			padded_x[i][:] = padded_x[i][:] / padded_x[i][at90]
